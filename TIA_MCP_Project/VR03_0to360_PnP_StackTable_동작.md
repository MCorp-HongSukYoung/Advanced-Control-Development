...
|---|---|---|---|
| **LC (Cathode)** | **`PP1X_GET` / `PP1Z_GET`** ← 마그자인에서 집기 | `PP2X_PUT` / `PP2Z_PUT` → Stack Table 에 놓기 | `ReverseZ_PUT` / `ReverseR_PUT` (PP1 → Reverse 로 **PUT 받음**) |
| **RA (Anode)** | `PP1X_PUT` / `PP1Z_PUT` → Stack Table 에 놓기 | **`PP2X_GET` / `PP2Z_GET`** ← 마그자인에서 집기 | `ReverseZ_GET` / `ReverseR_GET` (Reverse → PP1 으로 **GET 보냄**) |
...
|---|---|---|---|---|
| **0°** | — (StandStill 확인 후 진입) | PP1 = LC 마그자인 위 Z 하강 시작 (GET) ／ PP2 = LC Reverse 위 Z 하강 시작 (반전된 전극 GET) | PP1 = RA Reverse 위 Z 하강 시작 (반전된 전극 GET) ／ PP2 = RA 마그자인 위 Z 하강 시작 (GET) | Z 상승, 분리막 안착 자세 |
...
| **320° ~ 360°** | — | LC PP1/PP2 X 복귀 (PP1 → 다음 사이클 마그자인 위치, PP2 → 다음 사이클 Reverse 위치) | RA PP1/PP2 X 복귀 (PP1 → 다음 사이클 Reverse 위치, PP2 → 다음 사이클 마그자인 위치) | Z 원위치 복귀 |
...
1. **0° ~ 80°  · GET 단계** : `LC PP1` (Axis 13 X / 14 Z) 가 LC 마그자인 위로 X 이동 → Z 하강 → 흡착 → Z 상승.
...
8. **320° ~ 360°  · 복귀** : `LC PP1` 은 다음 사이클 마그자인 위로, `LC PP2` 는 다음 사이클 Reverse 위로 X 복귀.

### 4-2. RA (Anode) 흐름 — `RA PP2 GET` → `Reverse 반전` → `RA PP1 PUT`

LC 와 거울 대칭. **PP1/PP2 의 GET/PUT 역할이 반대** 임에 주의.

1. **0° ~ 120°  · GET 단계** : `RA PP2` (Axis 30 X / 31 Z) 가 RA 마그자인에서 Anode 흡착.
...
7. **320° ~ 360°  · 복귀** : `RA PP1` 은 다음 사이클 Reverse 위로, `RA PP2` 는 다음 사이클 마그자인 위로 X 복귀.
